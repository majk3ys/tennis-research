...
hard_elo = get_json("https://www.ultimatetennisstatistics.com/rankingsTableTable?current=1&rowCount=-1&searchPhrase=&rankType={0}_ELO_RANK&season=&date=&_=1580455394238".format('HARD'))
grass_elo = get_json("https://www.ultimatetennisstatistics.com/rankingsTableTable?current=1&rowCount=-1&searchPhrase=&rankType={0}_ELO_RANK&season=&date=&_=1580455394238".format('GRASS'))
clay_elo = get_json("https://www.ultimatetennisstatistics.com/rankingsTableTable?current=1&rowCount=-1&searchPhrase=&rankType={0}_ELO_RANK&season=&date=&_=1580455394238".format('CLAY'))
...
def reg_model(elo_prob, utr_prob, surf_rec, form, opp_hist):
    ...
    #print(0.35*elo_prob, 0.15*utr_prob, 0.2*surf_rec, 0.2*form, 0.1*opp_hist)
    return (0.35*elo_prob + 0.15*utr_prob + 0.2*surf_rec + 0.2*form + 0.1*opp_hist)
...
def elo_pwin(elo, player_name, opp_name, curr_date, surface, bet=False):    
    ...
    if bet:
        ...
        #surf_opp = surf_elo[(surf_elo['name'].str.split().str[-1]==opp_surname) & (surf_elo['name'].str.contains(opp_first_name))]

        if player_name == 'Ze Zhang':
            player_first_name = 'Ze'
        elif opp_name == 'Ze Zhang':
            opp_first_name = 'Ze'
            
        if player_name == 'Zhizhen Zhang':
             player_first_name = 'Zhi'
        elif opp_name == 'Zhizhen Zhang':
             opp_first_name = 'Zhi'
        ...
        #surf_opp = surf_elo[(surf_elo['name'].str.split().str[-1]==opp_surname) & (surf_elo['name'].str.contains(opp_first_name))]
    player = elo[(elo['name'].str.split().str[-1]==player_surname) & (elo['name'].str.contains(player_first_name))]
    surf_player = surf_elo[(surf_elo['name'].str.split().str[-1]==player_surname) & (surf_elo['name'].str.contains(player_first_name))]
    opp = elo[(elo['name'].str.split().str[-1]==opp_surname) & (elo['name'].str.contains(opp_first_name))]
    surf_opp = surf_elo[(surf_elo['name'].str.split().str[-1]==opp_surname) & (surf_elo['name'].str.contains(opp_first_name))]
    ...
    if len(player) == 0:
        ...
        if len(surf_player) > 0 and len(surf_opp) > 0:
            elo_player = (player['points'].values[0] + surf_player['points'].values[0])/2
            elo_opp = (opp['points'].values[0] + surf_opp['points'].values[0])/2
        else:
        ...
        if get_home_adv('New York', list(ranking_player['country'].iloc[0].values())):
            pwin += 0.1
        """
...
for index, row in ausopen_2019_matches.iterrows():
    ...
    #bet_data_match = ausopen_2019_matches[(ausopen_2019_matches['Winner'].apply(lambda x: x.split(' ')[0] in opp_name)) & (ausopen_2019_matches['Loser'].apply(lambda x: x.split(' ')[0] in player_name))]
    odds_player = row['AvgL']
    odds_opp = row['AvgW']
    ...
    if pred == opp_name:
        num_correct_pred += 1
        if prob_opp_win != 1:
            val_opp_win = round(odds_opp*prob_opp_win, 2)
            if 1.1 <= val_opp_win <= 2 and prob_opp_win >= 0.6:
                profit += odds_opp*stake - stake
                total_staked += stake
                win += 1
                print('WIN: Correctly bet {0} to beat {1}'.format(opp_name, player_name))
                print('My prob: {0}, market prob: {1}'.format(prob_opp_win, 1/odds_opp))
                #print(val_opp_win)
                
    # Incorrectly predict player to win
    else:
        if prob_player_win != 1:
            val_player_win = round(odds_player*prob_player_win, 2)
            if 1.1 <= val_player_win <= 2 and prob_player_win >= 0.6:
                profit -= stake
                total_staked += stake
                total_loss -= stake
                loss += 1
                print('LOSS: Incorrectly bet {0} to beat {1}'.format(player_name, opp_name))
                print('My prob: {0}, market prob: {1}'.format(prob_player_win, 1/odds_player))
